...
## 2. Результаты Моделирования (MVP)

| Метрика   | Значение | Интерпретация                                    |
|-----------|----------|-------------------------------------------------|
| $R^2$Score | 0.31     | Модель объясняет 31% вариативности конечной цены. |
| MAE       | 5.8 %    | Средняя абсолютная ошибка прогноза.              |
...
| Сырое Поле      | Планируемые Признаки                                                      | Метод             |
|-----------------|-------------------------------------------------------------------------|-------------------|
| CustomerINN_X3  | Паттерн Заказчика (AVG(reduction)), Опыт Заказчика (COUNT(tenders))      | GROUP BY + AVG/COUNT |
| WinnerINN_X4    | Паттерн Поставщика (AVG(reduction)), Опыт Поставщика (COUNT(wins))        | GROUP BY + AVG/COUNT |
| ContractDate_X8 | Сезонность (Месяц/Квартал)                                               | EXTRACT(MONTH/QUARTER) |




# 📝 Аналитический отчёт: прогноз снижения цены контракта (тендеры 44‑ФЗ)

## 1. Очищенные данные и переменные

Для анализа используется набор исторических данных по государственным тендерам.

| Столбец                         | Описание                                      | Тип      | Роль в модели             |
|---------------------------------|-----------------------------------------------|----------|---------------------------|
| `nmck_x1`                       | Начальная максимальная цена контракта (НМЦК)  | Числовой | Расчёт целевой `y`        |
| `finalprice_y`                  | Финальная цена контракта                      | Числовой | Расчёт целевой `y`        |
| `participants_x2`               | Количество участников тендера                 | Числовой | Признак (`X`)             |
| `y_reduction_percent_calculated`| Процент снижения цены                         | Числовой | Целевая переменная (`Y`)  |

**Очистка данных:** перед моделированием удаляются строки с `NaN`/`Inf` в столбцах `nmck_x1`, `finalprice_y`, `participants_x2` и `y_reduction_percent_calculated`.

---

## 2. Выбор модели

Для прогноза используется **линейная регрессия (`LinearRegression`)**:

- Модель проста и интерпретируема, быстро показывает наличие/отсутствие линейной связи между `participants_x2` и `y_reduction_percent_calculated`.
- Служит baseline: если результат слабый, значит признак или связь не подходят, и нужно усложнять модель и/или добавлять признаки.
